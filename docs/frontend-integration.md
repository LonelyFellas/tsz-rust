...
### 18.0 2026-09-13 完整分页修订（以下历史说明以本节为准）

- `component-targets/search` 新增可选请求字段 `entry_id`：查询前限定词条，仍应用 `q`、`match`、类型、草稿与归档条件。短语成分已知目标身份时传入该字段，目标内部候选仍通过 `next_cursor` 遍历。
- 移除 2000 词面/200 词条的**总量**窗口。只保留每批最多读取 200 个词条快照，统计完整候选数后分页；`page_size` 仍默认 50、最大 200，单位为候选而非去重词条。
- `total` 为完整候选数；`truncated=true` 时一定返回 `next_cursor`。新前端遇到旧服务 `truncated && !next_cursor` 应提示服务暂不能继续加载，不能提示修改固定成分词面。
- 稳定排序增加 entry_id 与词性/原形/变体身份。游标绑定 q/kind/match/include_drafts/entry_id、generation 与可用词条集合；归档造成结果集变化也会返回 `400 invalid_query` / `field=cursor`。前端提示重新加载，保留宿主已有关联。
- 每页仍统计完整匹配集，但遍历阶段只保留轻量候选身份与排序字段，分页后才为当前页物化包含
  `forms / senses` 的富候选；词面和快照也按词条批次读取。1001 词条/8008 候选的隔离 handler
  验证逐50条完整遍历约39秒；不等同于生产负载基准。常驻回归使用251个真实发布词条，覆盖旧200词条窗口。
- **本次后端先发布、前端后发布**：没有新增响应字段，但旧后端拒绝新增请求字段 entry_id。下文18.4的前端优先顺序仅对应早期新增响应 next_cursor 的版本。
...
- 连读是「两点之间的一条连线」，规范化**不合并**：首尾相接（连读链）、交叠、同区间不同端宽的
  连读各自保留，只去掉 `start` / `end` / `start_len` / `end_len` 四者全同的重复；
  emphasis / highlight 的合并规则不变。
...
迁移 `20260917180000_add_form_variant_regularity` 新增关系表 nullable boolean 列，并回填旧组值。
