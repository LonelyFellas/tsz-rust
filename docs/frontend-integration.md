...
### 24.2 同原型组的定义与标注权限（2026-09-07 修正）

**同原型组 = 当前管理员自己的词条（草稿或已发布） + 所有人的已发布词条。**
别人的草稿不进组——草稿只是「尚未公开」的状态，不构成需要靠标注区分的歧义。
对应谓词：

```sql
((source.content_scope = 'draft' AND entry.created_by_admin_id = $actor)
...
> 修正记录：2026-09-06 的 [#115](https://github.com/LonelyFellas/tsz-rust/pull/115)
> 曾把徽标改成 `source.source_revision = entry.revision`（不按创建人），与写路径分家。
> 那是错的，本次一并改回，别再按「与管理列表同口径」的理由退回去。
...
1. **撞上别人的词条、对方还没标注时照样能建**。当前管理员只填自己那条即可，不会因为
   改不了别人的而被卡住。
2. **冲突弹窗可以显示别人的词条，但只显示已发布的**；别人的草稿不显示。
3. **别人的词条在列表里能看到角标，只是没有编辑入口**——角标可见 ≠ 有编辑权。

#### 建条冲突不再要求填满全组

冲突响应里的 `entries` 仍然列出整组（包括别人的已发布词条），那是给前端**只读展示、避免
...
因为组的定义含「自己的草稿」，A 和 B 各有自己的草稿时，两人看到的角标可以不同——
...
`node_id` 就是变体 id，定位方式与 `voice_profile_invalid` 完全一致，前端的 `issueNavigation` 不用改。
