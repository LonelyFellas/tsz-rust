...
| 2 | 关联词搜索 / 例句目标发现要不要搜得到别人的草稿 | ~~**不要**。草稿只能被看见，不能被直接引用~~ **2026-09-11 推翻**：关联词搜索改为放开，见文末补记；例句目标发现维持不放开 |
...
**2026-09-08 用户确认：本次不做，留待后续做权限系统时一并处理。** 不是遗漏。

不动的理由（评估时的完整版）：
1. 标注不是普通字段，是**同名词条的区分标记**——「独立词条共享原型时，每个直接同名组内
   须填写非空、不同的标注」。改一条约束的是整个同名组（冲突响应会把整组列出来一起填），
   与改正文（只影响自己）不是一类操作。
2. 标注归属规则是 2026-09-07 定的，比本次口径早一天；推翻它需要明确的产品决策，
   不能从「已发布全员可编辑」顺手推导。

代价是承认的：别人已发布的词条，正文能改、能发布、能归档，唯独标注是灰的，
现在的置灰提示（「他人词条」）也没解释为什么标注特殊。做权限系统时一并收口——
方向倾向于放开（组内唯一本来就由后端兜底），或者至少把理由写进提示。
...
## 补记：2026-09-11 关联词搜索放开

用户推翻了决定 #2 的关联词那一半：别人建的草稿不仅要能看见，还要能被引用为
近义词、反义词或派生词。实施只去掉 `related-search` 草稿分支的创建者过滤
（`repository/query.rs`），写入与发布链路本就不带 creator 谓词，无需改动。

**未随之放开的**：例句目标发现（`sentence-targets/resolve`）维持只对创建者可见，
它已无前端入口；成分目标搜索维持只搜已发布；草稿写权限仍限创建者与超管。

**已知代价**：A 引用 B 的草稿词义后，草稿保存期就会在 `lexicon.relations` 落行，
于是 B 删不掉那条草稿、删不掉被引用的词义，A 发布后连归档也做不了；而当 A 的引用方
自身还是草稿时，B 无权编辑 A 的草稿来解绑，只能找 A 本人或超管。这是放开引用的直接
后果，未在本次提供自助解绑途径。
