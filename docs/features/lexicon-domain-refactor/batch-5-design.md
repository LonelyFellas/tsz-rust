...
状态：2026-09-23 两仓初版已提交，前端已推送并创建 PR #321；后端首轮审查发现的两项问题已补失败复现并修复，等待增量审查与推送。未合并或部署。第四批线上全部业务组合的人工验收不由本轮本地验证替代。
...
| 音频保护 | 草稿/发布结构化引用、七天宽限期、FOR SHARE / SKIP LOCKED、失败保留资产行 | 持资产排他锁后，用新语句快照再次查引用；先提交持久回收标记再删除对象，失败不重新开放引用；补历史非当前版本的保护、回退原资产 ID、试听 URL 与草稿不覆盖断言 |
...
| 契约 | 严格 OpenAPI、wire 类型和 runtime schema | 同步新增请求字段、生成快照与 schema 来源 SHA；响应形状不变；新增内部音频回收状态迁移 `20260923050000` |
...
- 用户已授权两仓提交、推送与 PR；前端 PR #321 已建立，后端增量审查后推送。未合并、部署或操作服务器数据。
...
- P1 先红：MemoryAdapter 实际删除对象后返回错误，原实现仍允许保存引用并返回 200。修复后先提交 `reclamation_started_at`，第二事务持锁执行可重试删除；超时、错误、提交失败或进程中断均不能撤回已提交标记。保存返回 `422 audio_asset_invalid`，试听/confirm 不再暴露标记资产，引用表触发器也拒绝绕过服务校验的写入。
- 新增迁移 `20260923050000_audio_asset_reclamation`（up/down 配对）。独立数据库音频套件 18 项通过，包括删除响应失败、对象确已不存在、禁止保存/直接引用/重放、后续幂等回收，以及迁移 down/up 和存在回收中资产时 down 原子拒绝。
- P2 先红：省略 headwords、noun=practice 与 verb UK=practise/US=practice 时，原实现漏查实际原型并返回 201。改用原始 headword keys 与最终 forms 中全部 base 原型的规范化方言 keys 并集；普通词形不加入。精确用例通过，缺说明拒绝且不部分修改旧标注，合法说明创建新 ID。
- 新后端依赖新列，不兼容旧 schema；升级须先停止旧业务写入及音频回收 worker，再执行迁移、替换新后端与配套前端后恢复。旧二进制能读新增列的数据库但不理解回收状态，不能在新 worker 产生标记后回退运行；数据库触发器仅保护引用，不能把旧试听/confirm 视作安全混部。
- 回退须停止全部写入和 worker，先由新版本完成所有回收中资产的幂等删除，再检查标记数为零，执行带守卫的 down 并配套回退组件。删除结果未知时保留状态，不以清空标记或强制 down 逃过保护。本次仅在隔离测试库验证，无线上 migration 操作。
- 此修复不改变 wire DTO/OpenAPI；前端代码和契约生成物无需再次变化。首轮浏览器验证仍是初版证据，补修证据来自上述真实数据库/handler 失败注入，不宣称生产 OSS 验收。
