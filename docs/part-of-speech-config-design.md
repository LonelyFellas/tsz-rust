...
| `code`                | TEXT        | NOT NULL，由固定名字的唯一索引保证全局唯一，创建后不可修改；短语词性必须以 `phrase_` 开头 |
...
- `code` 全局唯一（跨 kind）；短语词性的 `code` 以 `phrase_` 开头（CHECK `catalog_parts_of_speech_phrase_code_check`）；
...
- `(id, kind)` 另有唯一约束 `catalog_parts_of_speech_id_kind_key`，是 `lexicon.entry_pos` 与
  `catalog.form_types` 复合外键的引用目标：单词词条只能挂单词词性，词形变化只能挂单词词性；
...
非基础种子已由 `20260906120000` 迁移下线，管理员可按需自建，自建的词性同样可以扩展细分词性）。
...
**词形变化归属**（2026-09-10 起）：词形变化不再是全局目录，每条挂在一个基本词性下，
由 `catalog.form_types.part_of_speech_id` 记录；原形是唯一例外，对所有词性通用（该列为 null，
并由 CHECK 约束保证只有原形如此）。展示字段的唯一性收敛到同一基本词性内，所以形容词与副词
可以各有一个「比较级」；编码仍全局唯一，新建时由「父词性编码前 8 字 + 英文全称」派生。
`allowed_form_types` 随之按词性收窄，词条侧不做硬校验，存量词条不受影响。
删除基本词性前要求先清空它名下的词形变化（409 `part_of_speech_has_form_types`）。

**细分词性必填规则**：扩展细分词性不设限，任意基本词性都可以挂，`sub_parts_extensible` 恒为 true。
只有这五个编码下的释义必须选中细分词性：规则不落库，由 `catalog::rules::requires_sub_pos` 按 `code`
派生成 `sub_pos_required` 随管理项与 catalog 项下发，其余词性的 `sub_pos` 选填（填了仍校验归属）。
