...
| DB | pgx/v5 手写 SQL | **sqlx**（postgres, runtime-tokio） | 编译期 `query!` 校验；连接池 `PgPool` |
| 迁移 | golang-migrate（26 个增量） | **sqlx::migrate!**（重画干净 schema） | 功能重写，不复用历史迁移 |
| 鉴权 | JWT HS256 | **jsonwebtoken** | 双身份两套签名 key |
| 口令哈希 | bcrypt | **bcrypt** crate | 参数与 go 保持一致 |
| 配置 | env | **serde + envy**（或 figment） | `Config::from_env()` |
| 日志 | slog（JSON） | **tracing + tracing-subscriber**（json layer） | |
| 指标 | Prometheus | **metrics + metrics-exporter-prometheus** | `/metrics` |
| 链路 | OTel OTLP | **tracing-opentelemetry + opentelemetry-otlp** | opt-in |
| 校验 | validator/v10 | **validator** crate + 手写 | |
| UUID | google/uuid | **uuid**（v4, serde） | |
...
对标 Go 的架构约定：
- 依赖方向向内：`handler → service → repository`。
- repository 用 **trait** 抽象（对标 Go 的接口 + fake），service 依赖 trait。
- 每个有状态模块一套 **contract test**：同一批断言同时跑 fake 与真 Postgres（对标 `runStoreContract`）。
...
2. **sqlx 编译期校验需要一个可连的库**（或 `sqlx prepare` 生成 `.sqlx` 离线缓存）—— Day 1 先把离线缓存/CI 方案定好，否则 CI 无库会编译失败。
...
5. **契约测试的 fault-injection hook**（go 的 `*Fn`/`*Err`）不进契约——Rust 侧用 trait 默认实现或独立 mock。
