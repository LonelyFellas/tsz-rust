...
2. 在新上下文运行只读独立 reviewer：调用 `code-review` skill，target 传 `review_base...review_sha`，effort 至少 `high`；该 skill 不可用时创建独立 reviewer 子任务。传入原始目标、base、精确 SHA、仓库规则和测试结果，不传入实现者的审查结论。
