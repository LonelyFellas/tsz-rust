...
use super::model::{
    Actor, CreatePartRequest, DeleteRevisionQuery, PartListFilter, PartListQuery, PartPath,
    UpdatePartRequest,
};
use crate::{
...
#[utoipa::path(get, path="/api/v1/admin/settings/form-types", tag="admin-catalog", security(("bearer_auth"=[])), params(PartListQuery), responses((status=200, body=PaginatedResponse<FormTypeConfig>), (status=400, description="查询参数非法"), (status=403, description="需要超级管理员")))]
pub async fn list(
    ...
    let total = sqlx::query_scalar::<_, i64>(concat!("SELECT count(*) FROM catalog.form_types f", " WHERE $1::text IS NULL OR strpos(lower(concat_ws(' ',f.code,f.name_zh,f.name_en,f.short_name_zh,f.abbreviation,f.full_name_en)),lower($1)) > 0"))
        .bind(&filter.q).fetch_one(&mut *tx).await.map_err(database_error)?;
    let items = sqlx::query_scalar::<_, SqlJson<FormTypeConfig>>(concat!(select_config!(), " WHERE $1::text IS NULL OR strpos(lower(concat_ws(' ',f.code,f.name_zh,f.name_en,f.short_name_zh,f.abbreviation,f.full_name_en)),lower($1)) > 0", " ORDER BY f.sort_order,f.created_at,f.id LIMIT $2 OFFSET $3"))
        .bind(&filter.q).bind(filter.limit()).bind(filter.offset()).fetch_all(&mut *tx).await.map_err(database_error)?
...
#[utoipa::path(post, path="/api/v1/admin/settings/form-types", tag="admin-catalog", security(("bearer_auth"=[])), request_body=CreatePartRequest, responses((status=201, body=FormTypeConfig), (status=400, description="字段非法"), (status=403, description="需要超级管理员"), (status=409, description="配置冲突")))]
pub async fn create(
    State(state): State<AppState>,
    auth: AdminAuth,
    ...
    let mut tx = state.pool.begin().await.map_err(database_error)?;
    let id = Uuid::now_v7();
    sqlx::query("INSERT INTO catalog.form_types(id,code,name_zh,name_en,short_name_zh,abbreviation,full_name_en,sort_order,created_by_admin_id) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)")
        .bind(id).bind(request.code).bind(text(request.name_zh,"name_zh",64)?).bind(text(request.name_en,"name_en",64)?)
...
#[utoipa::path(patch, path="/api/v1/admin/settings/form-types/{id}", tag="admin-catalog", security(("bearer_auth"=[])), params(PartPath), request_body=UpdatePartRequest, responses((status=200, body=FormTypeConfig), (status=400, description="字段非法"), (status=403, description="需要超级管理员"), (status=404, description="配置不存在"), (status=409, description="配置或版本冲突")))]
pub async fn update(
    State(state): State<AppState>,
    auth: AdminAuth,
    ...
    let admin = require_super_admin(&state, &auth).await?;
    let mut tx = state.pool.begin().await.map_err(database_error)?;
    lock(&mut tx, path.id, request.base_revision).await?;
    sqlx::query("UPDATE catalog.form_types SET name_zh=$2,name_en=$3,short_name_zh=$4,abbreviation=$5,full_name_en=$6,sort_order=$7,updated_by_admin_id=$8,updated_at=now(),revision=revision+1 WHERE id=$1")
