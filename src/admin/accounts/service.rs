...
use crate::{
    admin::{
        AdminRole, NewAdmin,
        accounts::{
            AdminAccountResponse, AdminAccountsRepository, AdminAccountsRepositoryError,
            model::{
                AdminAccountListFilter, AdminCreatorResponse, AdminListQuery, AdminListResponse,
            },
        },
    },
...
impl AdminAccountsService {
    pub fn new(repository: AdminAccountsRepository) -> Self {
        Self { repository }
    }
    pub async fn provision(
        ...
        phone: &str,
        display_name: &str,
    ) -> Result<(AdminAccountResponse, String), AdminAccountsServiceError> {
        // 1. 生成临时密码
    ...
    pub async fn list(
        ...
        let filter = AdminAccountListFilter {
